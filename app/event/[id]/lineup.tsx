...
export default function LineupScreen() {
  ...
  const { data: event, isLoading: loading, error: eventError } = useEvent(id as string);
  ...
  const handleTabPress = (index: number) => {
    ...
    setTimeout(() => { isTappingTab.current = false; }, 500);
  };
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <TouchableRipple
                  ...
                  style={[styles.dayItem, selectedDateIndex === index && { borderBottomColor: theme.colors.primary, borderBottomWidth: 2 }]}
                  rippleColor="rgba(0,0,0,0.1)"
                >
                  <Text
                    variant="titleMedium"
                    style={[selectedDateIndex === index ? { color: theme.colors.primary, fontWeight: "bold" } : { color: theme.colors.onSurface, opacity: 0.6 }]}
                  >
                    {(() => {
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <TouchableRipple
                  ...
                  style={[styles.tabItem, activeTabIndex === index ? { backgroundColor: theme.colors.primary } : { backgroundColor: addAlpha(theme.colors.onSurface, 0.05) }]}
                  rippleColor="rgba(255,255,255,0.2)"
                >
                  <Text variant="labelLarge" style={[styles.tabText, activeTabIndex === index ? { color: theme.colors.onPrimary } : { color: theme.colors.onSurface }]}>
                    {item.name}
                  </Text>
                </TouchableRipple>
              )}
...
const styles = StyleSheet.create({
  ...
  dayContainer: { borderBottomWidth: 1, borderBottomColor: "rgba(0,0,0,0.05)", marginBottom: 8 },
  ...
  actCard: { borderRadius: 12, overflow: "hidden" },
  actRipple: { padding: 12 },
  actContent: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  actName: { fontWeight: "700", flex: 1 },
  actTime: { opacity: 0.6, fontWeight: "500" },
  emptyContainer: { flex: 1, justifyContent: "center", alignItems: "center", marginTop: 100 },
  liveBadge: { flexDirection: "row", alignItems: "center", paddingHorizontal: 8, paddingVertical: 4, backgroundColor: "rgba(255,0,0,0.1)", borderRadius: 12, gap: 4 },
